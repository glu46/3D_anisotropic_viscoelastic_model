...
C
		...
		A4 = 140.D0
C		
		IF (COORDS(3).LE. A1) THEN
C		
			ISWITCH = 18*4
C
		ELSEIF (COORDS(3).LE. A2) THEN
C			
			ISWITCH = 18*3
C		
		ELSEIF (COORDS(3).LE. A3) THEN
C			
			ISWITCH = 18*2
C			
		ELSEIF (COORDS(3).LE. A4) THEN
C			
			ISWITCH = 18
C		
		ELSE
...
C			
		...
		XMATD = 0.D0
		XMATD(1,1) = PROPS(1+ISWITCH)
		XMATD(2,2) = PROPS(2+ISWITCH)
		XMATD(3,3) = PROPS(3+ISWITCH)
		XMATD(4,4) = PROPS(6+ISWITCH)
		XMATD(5,5) = PROPS(5+ISWITCH)
		XMATD(6,6) = PROPS(4+ISWITCH)
		XMATD(1,2) = PROPS(7+ISWITCH)
		XMATD(1,3) = PROPS(8+ISWITCH)
		XMATD(2,3) = PROPS(9+ISWITCH)
		XMATD(2,1) = XMATD(1,2)
		XMATD(3,1) = XMATD(1,3)
		XMATD(3,2) = XMATD(2,3)
C===========================================		
...
C===========================================
		XN = 0.D0
		XN(1,1) = PROPS(1+9+ISWITCH)
		XN(2,2) = PROPS(2+9+ISWITCH)
		XN(3,3) = PROPS(3+9+ISWITCH)
		XN(4,4) = PROPS(6+9+ISWITCH)
		XN(5,5) = PROPS(5+9+ISWITCH)
		XN(6,6) = PROPS(4+9+ISWITCH)
		XN(1,2) = PROPS(7+9+ISWITCH)
		XN(1,3) = PROPS(8+9+ISWITCH)
		XN(2,3) = PROPS(9+9+ISWITCH)
		XN(2,1) = XN(1,2)
		XN(3,1) = XN(1,3)
		XN(3,2) = XN(2,3)
