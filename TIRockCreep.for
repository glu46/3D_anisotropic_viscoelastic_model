...
C===========================================	
C	  XMATD STORED B_IJ IN EQ.(7B); EQ(8) AND (9)
C      CHARACTERISE ORTHO/TI MATERIALS WITH FEWER PARAMETERS.	
C===========================================
		XMATD = 0.D0
		XMATD(1,1) = PROPS(1)
		XMATD(2,2) = PROPS(2)
		...
		XMATD(3,2) = XMATD(2,3)
C===========================================		
C       XN STORED THE POWER INDICES N_IJ IN EQ.(7B); 
C         EQ(8) AND (9) CHARACTERISE ORTHO/TI MATERIALS
C         WITH FEWER PARAMETERS.
C===========================================
		XN = 0.D0
		XN(1,1) = PROPS(1+9)
		XN(2,2) = PROPS(2+9)
...
C	  
		ROT = 3.D0*PI/6.D0
C===========================================		
C		ROT, ROTATION OF A TI MATERIAL'S BEDDING PLANE,
C       	IS POSITIVE IF FROM X TO X' IS CLOCKWISE.
C       IN THIS CASE, Y IS IDENTICAL TO Y'.
C===========================================
C===========================================		
C		CALCULATE COSINES IN EQ.(B1)
C           FOR TRANSFORMATION MATRIX
C       THIS IS AN EXAMPLE FOR A TI MATERIAL,
C           WHOSE BEDDING PLANE ROTATE ABOUT Y AXIS. 
C===========================================
		XCOSL = 0.D0
		XCOSM = 0.D0
		XCOSN = 0.D0
...
C=============================================		
		...
		TRANSFORM_T(5,4)=XCOSL(2)*XCOSM(3)+XCOSL(3)*XCOSM(2)
		TRANSFORM_T(6,4)=XCOSL(1)*XCOSM(3)+XCOSL(3)*XCOSM(1)
		...
		TRANSFORM_T(5,5)=XCOSM(2)*XCOSN(3)+XCOSM(3)*XCOSN(2)
		TRANSFORM_T(6,5)=XCOSM(1)*XCOSN(3)+XCOSM(3)*XCOSN(1)
		...
		TRANSFORM_T(5,6)=XCOSL(2)*XCOSN(3)+XCOSL(3)*XCOSN(2)
		TRANSFORM_T(6,6)=XCOSL(1)*XCOSN(3)+XCOSL(3)*XCOSN(1)
C		
C===========================================
